function Breakthrough_Time_Test
    R      = 8.314;
    T_in   = 298.0;
    Pbar   = 4.5;
    P_in   = Pbar*1e5;
    ...
    [Tsol, Y] = ode15s(@(t,y) rhs_yN2_xT(t,y,pars), [0 tend], y0, opts);

    yN2_out_trace = Y(:, 1:pars.Nx);
    yN2_out_trace = yN2_out_trace(:, end);
    thr10 = 0.10*yN2_in; thr50 = 0.50*yN2_in;

    tB10 = localFirstCross(yN2_out_trace, Tsol, thr10);
    tB50 = localFirstCross(yN2_out_trace, Tsol, thr50);

    MTZ = NaN;
    if ~isnan(tB50)
        [~,k] = min(abs(Tsol - tB50));
        yprof = (Y(k,1:Nx).'/yN2_in);
        xgrid = linspace(0,L,Nx).';
        x10 = interp1(yprof, xgrid, 0.10, 'linear','extrap');
        x90 = interp1(yprof, xgrid, 0.90, 'linear','extrap');
        MTZ = max(0, x90 - x10);
    end

    fprintf('Outlet yN2 range: [%.3f, %.3f]\n', min(yN2_out_trace), max(yN2_out_trace));
    fprintf('Breakthrough (10%% of feed N2): t_B10 = %.2f s\n', tB10);
    fprintf('Breakthrough (50%% of feed N2): t_B50 = %.2f s\n', tB50);
    if ~isnan(MTZ), fprintf('Estimated MTZ length at t_B50: %.3f m\n', MTZ); end

    yN2_end = Y(end,1:Nx).';  T_end = Y(end,3*Nx+1:4*Nx).';
    prof_end = compute_pressure_velocity_x(pars, yN2_end, T_end);
    Ctot_L = prof_end.Ctot(end);
    yO2_out = 1 - yN2_end(end);
    C_O2_out = yO2_out*Ctot_L; C_N2_out = (1-yO2_out)*Ctot_L;
    purityC = 100*C_O2_out/(C_O2_out + C_N2_out);
    fprintf('Outlet O2 concentration (x=L, final): %.2f mol/m^3; Purity: %.1f%%\n', C_O2_out, purityC);

    function tB = localFirstCross(ytrace, tvec, thr)
